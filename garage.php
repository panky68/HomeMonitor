<?php
    ...
    if(userLoggedIn() === true){    //user logged on?
        ...
        $last_reading = getLastReadings('garageSensorData');//Get Last Temparature, Humidity and Pressure inputs from dB
        ...
        foreach($esp32GPIOPins as $GPIOPin){                        //itterate through array
            ${'btn' . $arrayCount} = createBtn($GPIOPin);           //create button for each arduino gpio pin, var $btn(x) created
    ...
    <!-- Main Content -->
    <div class="container">
        <h2 class="pageHeader">Garage Page</h2> <!-- Page Title -->
        <div class="row">

            <!-- Camera Feed for Garage from rtsp server -->
            <div class="col-md-8">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://rtsp.me/embed/hGEsZBTS/" allowfullscreen allow="autoplay"></iframe>  <!-- garage ip camera feed from rtsp server -->
                </div>
            </div>

            <!-- Toggle button I/O Output switches -->
            <div class="col-md-4">
            ...
            $('.editSwitchLabel').click(function(){     //when any switch label has been clicked
                ...
                $.ajax({                                //query code for ajax
                    ...
                    success:function(data){             //if request succeeds, function is called
                        $('#labelDetails').html(data);  //display data in modal body in div tag #labelDetails
    ...
    <script>
        ...
        function updateOutput(element) { //HTTP Request to SET Toggle Switch Outputs
            ...
            if(element.checked){            //check if switch is on/off
                xhr.open("GET", "garageRoomOutputs.php?action=output_update&id="+element.id+"&state=1", true);  //set GET request to update switch switch to on
            }else{
                xhr.open("GET", "garageRoomOutputs.php?action=output_update&id="+element.id+"&state=0", true);  //set GET request to update switch switch to off
            }
